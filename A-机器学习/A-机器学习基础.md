...
- 给定学习任务，
  - 当训练不足时，模型的**拟合能力不够**（数据的扰动不足以使模型产生显著的变化，或者说模型学到的参数对数据不太敏感），此时**偏差**主导模型的泛化误差；
...
- 偏差和方差的关系和**模型容量**（模型复杂度）、**欠拟合**和**过拟合**的概念紧密相联
  ...
  - 为什么随着模型容量增大，模型的偏差减小、方差增大呢？偏差减小是因为，模型容量越大，模型空间越大(参数的搜索空间越大)，越有可能包含真实的函数。方差增大是因为，搜索出的参数对训练数据更加敏感。
  > 《深度学习》 5.4.4 权衡偏差和方差以最小化均方误差

### 如何判断模型过拟合
- 模型的error来自两个方面，模型的bias和variance。观察上图，当模型的variance主导模型的error时，表示已经过拟合了。
- 看模型能不能很好的拟合给定训练数据，如果不能，说明欠拟合。如果能够很好的拟合训练数据，测试error明显高于训练的error，表明过拟合。但是，高多少算是过拟合呢？

**Reference**
- [Understanding the Bias-Variance Tradeoff](http://scott.fortmann-roe.com/docs/BiasVariance.html)
- [机器学习中的Bias(偏差)，Error(误差)，和Variance(方差)有什么区别和联系？](https://www.zhihu.com/question/27068705) - 知乎 
- [李宏毅机器学习视频](https://www.bilibili.com/video/av59538266/?p=5)
